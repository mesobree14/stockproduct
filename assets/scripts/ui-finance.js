...
$(document).on("click", "#select_financedate", function (e) {
  Swal.fire({
    showConfirmButton: false,
    html: `
...
$(document).on("click", "#select_financedate_withdraw", function (e) {
  Swal.fire({
    showConfirmButton: false,
    html: `
      
      
      <form id="isPopupForm" class="mt-4 row text-center" method="POST" action="backend/PDF_withdrawdate.php" enctype="multipart/form-data" target="_blank">
        <div class="form-group col-6">
          <label for="" class="mr-auto str_date">เริ่มตั้งแต่วันที่</label>
          <input type="date" name="start_date" class="form-control" placeholder="Start Date" required>
        </div>
